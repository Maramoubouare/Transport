...
export const TrainSchedule = ({ route }) => {
  const { searchResults } = route.params;
  const [sortedResults, setSortedResults] = useState(searchResults.filter(item => item.type === 'train'));
  ...
  const handleResetFilter = () => {
    const busResults = searchResults.filter(item => item.type === 'train');
