...
export const AvionSchedule = ({ route }) => {
  const { searchResults } = route.params;
  const [sortedResults, setSortedResults] = useState(searchResults.filter(item => item.type === 'avion'));
  const [companyFilter, setCompanyFilter] = useState(null);
  const [isFilterResetVisible, setIsFilterResetVisible] = useState(false);
  const [isFiltered, setIsFiltered] = useState(false);
  const nav = useNavigation();
  const handleResetFilter = () => {
    const busResults = searchResults.filter(item => item.type === 'avion');
  ...
  const handleSortByPrice = () => {
    ...
    const filterByCompany = companyName => {
      const filtered = searchResults.filter(item => item.type === 'avion' && item.companyName === companyName);
  ...
  const scheduleItem = ({item}) => {
    return (
      ...
      <View style={styles.rightPart}>
        ...
        <TouchableOpacity style={styles.detailsButton} onPress={() =>  nav.navigate('info',{ item })}>
